...
class XyzReader():
    ...
    def extract_data_from_xyz_file(self, xyz_file_path):
        """
        ...
        with open(xyz_file_path, 'r') as f:
            for line in f:
                ...
                if len(split_line) == 4:
                    atom_with_counter = f"{split_line[0]}{counter:02}"
                    split_line[0] = atom_with_counter
                    l.append(split_line)
                    counter += 1
        return l
    ...
    def obtain_all_bond_orders(self, raw_coordinates):
        ...
        num_atoms = len(raw_coordinates)
        for i in range(num_atoms):
            atom1, x1, y1, z1 = raw_coordinates[i]
            for j in range(i + 1, num_atoms):
                atom2, x2, y2, z2 = raw_coordinates[j]
...
#     test = XyzReader()
#     raw_coords = test.extract_data_from_xyz_file(
#         "C:\\Documents\\Gaussian-2-Blender\\input_examples\\xyz files\\2-cyano-2-aminoaceticacid.xyz")
